...
export default function Photography({ service, services }) {
  return (
    <Layout>
      <Seo
...
export async function getStaticProps({ params }) {
  ...
  const services = servicesData.data.filter(
    (item) => item.attributes.title !== 'Photography'
  );

  const serviceRes = await fetch(
    'https://aztec.yeomedia.dev/api/services/4?populate[servicePageSlider][populate]=*&populate[image]=*&populate[fullWidthImage1]=*&populate[fullWidthImage2]=*&populate[textImage]=*&populate[accordions]=*&populate[servicePageBox]=*'
  );
